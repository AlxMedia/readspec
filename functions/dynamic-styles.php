...
/* ------------------------------------ */
if ( ! function_exists( 'readspec_hex2rgb' ) ) {

	function readspec_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'readspec_enqueue_google_fonts' ) ) {

	function readspec_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'readspec_enqueue_google_fonts' );


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'readspec_dynamic_css' ) ) {

	function readspec_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = readspec_hex2rgb($color_1);
			...
			// content width
			if ( get_theme_mod('content-width','780') != '780' ) {			
				$styles .= '.content-inner { max-width: '.esc_attr( get_theme_mod('content-width') ).'px; }'."\n";
			}
			// container width
			...
			// primary color
			if ( get_theme_mod('color-1','#2eb398') != '#2eb398' ) {
				$styles .= '
...
.wp-pagenavi span.current { border-bottom-color: '.esc_attr( get_theme_mod('color-1') ).'!important; }				
			...
			wp_add_inline_style( 'readspec-style', $styles );		
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'readspec_dynamic_css' );
